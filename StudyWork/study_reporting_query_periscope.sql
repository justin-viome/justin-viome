...
-- add studyid to info from chat flow requests for easier reporting 
	select case when cft.id=2 then 1
		...
		end as study_id, count(distinct cfr.user_id) as DistinctUsersContacted
...
-- get breakdown of study samples by type
	select study_id, count(distinct study_sample_id) filter (where st.sample_type_id=1) as StoolSamples,
		count(distinct study_sample_id) filter (where st.sample_type_id=2) as BloodSamples,
		count(distinct study_sample_id) filter (where st.sample_type_id=3) as SalivaSamples
...
-- get question and answer data 
	select sf.study_id, count(distinct sf.feature_id) as QuestionCount, count(a.*) as AnswerCount 
...
select s.study_id, s.name, s.description, s.Participant_Count,
	nt.DistinctUsersContacted as App_Users_Contacted_Via_Chat,
	samp.StoolSamples, samp.BloodSamples, samp.SalivaSamples,
	q.QuestionCount, q.AnswerCount 
from baseinfo s
left join notifications nt on nt.study_id=s.study_id
left join studsamples samp on samp.study_id=s.study_id
